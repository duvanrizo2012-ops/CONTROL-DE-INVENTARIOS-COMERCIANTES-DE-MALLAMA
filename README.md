# CONTROL DE INVENTARIOS COMERCIANTES DE MALLAMA

## DESCRIPCION DEL PROYECTO
El presente proyecto tiene como finalidad dar solución a la problemática
identificada en los comerciantes del municipio de Mallama, relacionada con la deficiente
gestión y control de inventarios, la cual genera desorganización, pérdidas económicas y
dificultades en la toma de decisiones. En la fase anterior, se desarrolló un proceso
investigativo que permitió analizar esta situación a través de métodos cuantitativos y
cualitativos, identificando sus principales causas y efectos, así como los requerimientos
necesarios para plantear una solución tecnológica adecuada.
A partir de estos resultados, en esta fase se da continuidad al proyecto mediante el
diseño y desarrollo de un prototipo funcional que materializa la propuesta planteada. Este
prototipo, desarrollado en la herramienta Figma, permite simular el funcionamiento de un
aplicativo móvil orientado al control de inventarios, integrando funcionalidades como el
registro, consulta, actualización, ventas y control de productos.
_ _ _
## INTEGRANTES
 - DUVAN DAVID RIZO
 - JAMES ANCIZAR PATIÑO

_ _ _

##
